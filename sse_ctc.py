...
# ----------------------
# Dataset generation (file_id, keywords, duration)
# ----------------------
def generate_dataset(num_files, num_keywords_per_file, max_duration_steps=5):
    dataset = []
    for i in range(num_files):
        file_id = f"file_{i}"
        keywords = [f"keyword_{j}" for j in range(num_keywords_per_file)]
        # assign a random validity measured in steps of delta
        d_steps = random.randint(1, max_duration_steps)
        dataset.append((file_id, keywords, d_steps))
    return dataset
...
if __name__ == "__main__":
    ...
    def benchmark_once(num_files: int, num_keywords_per_file: int, max_duration_steps: int,
                       delta_seconds: int, time0: int, query_keywords=None):
        # Setup entities
        do = DataOwner()
        _, ts = do.setup(delta_seconds, time0)
        server = CloudServer()
        # Build dataset
        DB = generate_dataset(num_files=num_files,
                              num_keywords_per_file=num_keywords_per_file,
        ...
        if query_keywords is None:
            # Use up to three keywords from synthetic naming scheme
            kcnt = min(3, num_keywords_per_file)
            query_keywords = [f"keyword_{i}" for i in range(kcnt)]
            # Ensure at least two keywords to exercise j>=2 loop
            if len(query_keywords) < 2:
                query_keywords.append("keyword_0")
        # Measure tokengen
    ...
    def benchmark_suite(configs, runs: int = 3):
        summary = []
        for cfg in configs:
            agg = {"construct_edb_ms": 0.0, "tokengen_ms": 0.0, "search_ms": 0.0}
            last_detail = None
            for _ in range(runs):
        ...
        for item in summary:
            print(
                f"files={item['num_files']}, kws/file={item['num_keywords_per_file']}, "
                f"d_max={item['max_duration_steps']}, delta={item['delta_seconds']}s | "
                f"EDB={item['construct_edb_ms']:.2f}ms, TOK={item['tokengen_ms']:.2f}ms, "
                f"SEARCH={item['search_ms']:.2f}ms (runs={item['runs']})"
            )

    # Default demo run
    delta = 60  # 60 seconds
    time0 = 1700000000
    print("\n--- Demo run (single query) ---")
    do = DataOwner()
    _, ts = do.setup(delta, time0)
    server = CloudServer()
    DB = generate_dataset(num_files=5, num_keywords_per_file=10)
    construct_edb(do, server, ts, DB, delta)
    qkws = ["keyword_0", "keyword_1", "keyword_2"]
    stag, t_len, xToken, time_prime, hc = tokengen(do, server, ts, qkws, delta)
    results = search(do, server, ts, qkws, stag, t_len, xToken, time_prime, hc)
    print("Search results:", results)

    # Benchmark suite examples
    print("\n--- Benchmark suite ---")
    configs = [
        {"num_files": 100, "num_keywords_per_file": 10, "max_duration_steps": 5, "delta_seconds": delta, "time0": time0},
        {"num_files": 500, "num_keywords_per_file": 10, "max_duration_steps": 5, "delta_seconds": delta, "time0": time0},
        {"num_files": 1000, "num_keywords_per_file": 10, "max_duration_steps": 5, "delta_seconds": delta, "time0": time0},
    ]
    benchmark_suite(configs, runs=3)
