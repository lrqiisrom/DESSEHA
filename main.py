...
do = DataOwner()
SK, server = do.setup()
do.add_docs([("d1", ["budget", "rom"]), ("d2", ["rom"]), ("d3", ["mky", "rom"])], SK, server)
A = User("A"); B = User("B"); D = User("D")
E = User("C")
A.enroll(); B.enroll(); D.enroll(); E.enroll()
OnlineAuth_DO_to_user(do, server, SK, A, ["d1", "d2"])
OnlineAuth_DO_to_user(do, server, SK, E, ["d1", "d3"])
OfflineAuth_A_to_B(server, A, B, ["d1", "d2", "d3"])
OfflineAuth_A_to_B(server, A, D, ["d2"])
res_B = Search("rom", B, server)
res_D = Search("rom", D, server)
res_E = Search("mky", E, server)
print("Authorized B search results:", res_B)
print("Unauthorized D search results:", res_D)
print("Unauthorized E search results:", res_E)
